...
if($userloggedin == 'support-service' || $userloggedin == 'pinky-a1' || $userloggedin == 'honeylyn-l1'){
  $query_date = mysqli_query($conn, "SELECT date_set, acc_amt FROM exp_monitor WHERE (user='support-service' || user='pinky-a1' || user='honeylyn-l1')");
  ...
  if(isset($_POST['submit'])){
    if(!empty($_POST['expense_limit'])){
      ...
      if($row=mysqli_num_rows($query_date) > 0) {
         $query = mysqli_query($conn, "UPDATE exp_monitor SET exp_limit='$exp_limit', exp_input='$exp_input', acc_amt='$acc_amt' WHERE (user='support-service' || user='pinky-a1' || user='honeylyn-l1')");
      }else{
...
<div class="container">
      ...
      <div class="p-2 mb-2 text-success main" style="background:#F2DDC1">
       <h5 class="item-gen">Expense Monitoring setup edit page</h5><a class="text-decoration-none" href="exp_monitor_esp"><h5 class="text-info item-gen">Expense monitor home
