...
$query_exp = mysqli_query($conn, "SELECT exp_limit, date_set, acc_amt FROM exp_monitor WHERE user='$userloggedin'");
...
if(isset($_POST['submit'])){
  if(!empty($_POST['expense'])){
    ...
    if($exp_limit > 1){
        if($row=mysqli_num_rows($query_exp) > 0) {
          $query_input = mysqli_query($conn, "UPDATE exp_monitor SET exp_input='$expense', acc_amt='$acc_amt' WHERE user='$userloggedin'");
        }else{
          header("Location: exp_monitor_edit?in=$expense");
        }
    }else{
      header("Location: exp_monitor_edit?in=$expense");
    }
...
<div class="container">
      ...
      <div class="p-2 mb-2 text-success main" style="background:#F2DDC1">
        ...
        <div class="item-gen d-flex justify-content-end flex-column">
          <a class="text-decoration-none" href="exp_monitor_edit"><h5 class="text-info float-right">Set limit
     ...
     <form class="" action="" method="POST">
       <div class="p-2 py-3 mb-2 form-group" style="background:#FFAFAF">
         ...
         <input type="text" name="expense" placeholder="Input expense" class="ml-2 border runded d-inline-block">
...
<script type="text/javascript">
  $(function(){
   ...
   $('#reset').on('click', function(){
    $.ajax({
      type:'POST',
      url: '../../includes/form_handlers/reset_exp_monitoring.php',
      data:{'userlogin':userlogin},
      cache:false,
      "success":function(data, textStatus){
        alert(data);
        window.location.href='exp_monitor';
      }
    });
  });
