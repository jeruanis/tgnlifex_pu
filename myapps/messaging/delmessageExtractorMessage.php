...
$get_deleted_query = mysqli_query($conn, "SELECT * FROM messages WHERE deleted='yes' ORDER BY id DESC");

   echo" <h1>This is from messages</h1>
    ...
    ";
        ...
        while($row=mysqli_fetch_assoc($get_deleted_query)){
            ...
            echo"<tr><td>$id</td><td >$body</td><td>$date</td><td>$image</td><td>$gif</td><td>$ageShow</td><td>$photoGif</td><td>$photoImage</td></tr>";
            ...
            if($ageShowNum >= 1) {
                ...
                $del_request = mysqli_query($conn, "DELETE FROM messages WHERE id='$id'");
