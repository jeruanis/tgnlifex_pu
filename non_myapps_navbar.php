 <?php  $curPageName = substr($_SERVER["SCRIPT_NAME"],strrpos($_SERVER["SCRIPT_NAME"],"/")+1);
    if ($curPageName == 'index.php')
			echo '<nav class="navbar navbar-expand-lg navbar-light bg-class py-3">';
		else
		  echo '<nav class="navbar navbar-expand-lg navbar-light bg-info py-3">';
	?>
                <!-- logo -->
		<button class="navbar-toggler border-0" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">

			<?php if ($curPageName != 'inventory_create_form.php' || $curPageName != 'house_needs_inventory_edit.php' || $curPageName != 'house_needs_inventory.php' || $curPageName != 'delete_member_inv.php' || $curPageName != 'add_member_inv.php'){ ?>
       ...
       <?php if ($curPageName != 'index.php' ) {  ?>
			...
			<li class="nav-item">
				<a class="nav-link" href="myapps/todo/todo" title="To do task">
			...
			<li class="nav-item">
		     ...
		     <li class="nav-item">
				<a class="nav-link"  href="myapps/main/request" title="Friend request">
				 ...
				 <div class="dropdown-menu" aria-labelledby="navbarDropdown" style="width:300px;max-height:400px;overflow:auto;">
					 <?php
						if ($curPageName != 'inventory_create_form.php' )
							 echo'<a id="ci" class="dropdown-item" href="myapps/inventory/inventory_create_form">Create inventory</a>';
							 echo '<hr>';
							include( "myapps/utilities/inventory_name_loader.php");
							echo '<hr>';
							if($userloggedin == 'support-service' || $userloggedin == "pinky-a1"){
								if(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on'){
									 $url=(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
										 if (strpos($url, '/group_messaging/') == false)
												 echo'<a class="dropdown-item" href="myapps/group_messaging/group_create_form">Create new chat group</a>';
										 echo '<hr>';
									 include( "myapps/utilities/group_name_loader.php");
								}else{
									 $url=(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
									 if (strpos($url, '/group_messaging/') == false)
											 echo'<a class="dropdown-item" href="myapps/group_messaging/group_create_form">Create new chat group</a>';
			 ...
			 <?php
				 if (isset($_SESSION['username'])){
					 ...
					 echo '<a class="ui image label" href="myapps/profile/' . $userloggedin . '" target="_blank">
							 <img src="../' . $profile_pic. '" alt="profile image">
							 '.$userloggedin.'
						 </a>';
					 echo '<hr>';
					 echo '<a href="myapps/userlogin/logoutbridge" title="logout">
				 ...
				 if (isset($_SESSION['username'])){
					 ...
					 echo '<a href="myapps/userlogin/logoutbridge" title="logout">
						 ...
						 echo '</div><div class="reg-out text-deoration-none"> <a href="myapps/userlogin/registration_signup_page">Welcome  Guest! Register | Login</a> </div>';
				 }
			 ?>
